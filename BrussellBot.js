...
bot.on("message", function (msg) {
	...
	if (msg.mentions.length !== 0) { //cleverbot
		msg.mentions.forEach(function(usr) { 
			if (usr.id == bot.user.id && msg.content.startsWith("<@"+bot.user.id+">")) { cleverbot(bot, msg); talkedToTimes += 1; console.log(checkForServer(msg)+colors.cGreen(msg.author.username)+" > "+colors.cYellow('@Bot-chan')+" "+msg.content.substring(22).replace(/\n/g, " ")); return; }
		});
	}
	...
	if (msg.content.startsWith(config.command_prefix)) { //normal commands
		if (commands.hasOwnProperty(cmd)) {
			try {
				console.log(checkForServer(msg)+colors.cGreen(msg.author.username)+" > "+msg.content.replace(/\n/g, " "));
				commandsProcessed += 1;
...
function checkForServer (msg) {
	if (msg) {
		if (!msg.channel.isPrivate) { return colors.cServer(msg.channel.server.name)+" > "; }
		else { return ""; }
	} return "";
}
		...
		try {
			...
			bot.joinServer(msg.content, function (err, server) {
				if (err) {
					...
					bot.sendMessage(msg, colors.cGreen("Successfully joined " + server.name));
