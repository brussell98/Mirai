...
bot.on("message", msg => {
	...
	if (msg.channel.isPrivate) {
		...
		if (msg.mentions.length !== 0) {
			...
			if (msg.isMentioned(bot.user) && new RegExp('^<@!?' + bot.user.id + '>').test(msg.content)) { //if mentioned first
				...
				if (config.send_mentions) {
					...
					if (owner && owner.status != "online") {
						...
						if (msg.channel.messages.length >= 3) {
							...
							bot.sendMessage(owner, `*In ${msg.channel.server.name}:*\n${toSend}`);
						} else bot.sendMessage(owner, `*In ${msg.channel.server.name}:*\n**${msg.author.username}:** ${msg.cleanContent.substr(0, 1930)}`);
					}
				}
			}
		}
	...
	if (msg.content.indexOf(" ") === 1 && msg.content.length > 2)
		msg.content = msg.content.replace(" ", ""); //if auto-inserted mobile space after prefix
	...
	if (msg.content.startsWith(config.command_prefix)) {
		...
		if (commands.commands.hasOwnProperty(cmd)) {
			if (!msg.channel.isPrivate)
				db.updateTimestamp(msg.channel.server); //update auto-leave timer
			execCommand(msg, cmd, suffix);
		} else if (commands.aliases.hasOwnProperty(cmd)) {
			if (!msg.channel.isPrivate)
				db.updateTimestamp(msg.channel.server); //update auto-leave timer
			msg.content = msg.content.replace(/[^ ]+ /, config.command_prefix + commands.aliases[cmd] + " "); //replace alias with command for hardcoded stuff
		...
		if (mod.commands.hasOwnProperty(cmd)) {
			...
			if (!msg.channel.isPrivate)
				db.updateTimestamp(msg.channel.server); //update auto-leave timer
			msg.content = msg.content.replace(/[^ ]+ /, config.mod_command_prefix + mod.aliases[cmd] + " "); //replace alias with command for hardcoded stuff
		...
		if (type == "normal") {
			...
			if (!msg.channel.isPrivate && commands.commands[cmd].hasOwnProperty("deleteCommand")) {
				if (commands.commands[cmd].deleteCommand === true && ServerSettings.hasOwnProperty(msg.channel.server.id) && ServerSettings[msg.channel.server.id].deleteCommands == true)
					bot.deleteMessage(msg, {"wait": 10000}); //delete command if needed
			}
...
bot.on("serverNewMember", (server, user) => {
	if (config.non_essential_event_listeners && ServerSettings.hasOwnProperty(server.id) && ServerSettings[server.id].welcome != "none") {
		if (!user.username || !ServerSettings[server.id].welcome || !server.name)
			return;
...
bot.on("userBanned", (user, server) => {
	if (config.non_essential_event_listeners && ServerSettings.hasOwnProperty(server.id) && ServerSettings[server.id].banAlerts == true) {
		...
		if (ServerSettings[server.id].notifyChannel != "general")
			bot.sendMessage(ServerSettings[server.id].notifyChannel, `⚠ ${user.username.replace(/@/g, '@\u200b')} was banned`);
		else
			bot.sendMessage(server.defaultChannel, `⚠ ${user.username.replace(/@/g, '@\u200b')} was banned`);
		bot.sendMessage(user, "🍌🔨 You were banned from " + server.name);
	}
});

bot.on("userUnbanned", (user, server) => {
	if (server.members.length <= 500 && config.non_essential_event_listeners)
		console.log(user.username + " unbanned on " + server.name);
});
...
bot.on("presence", (userOld, userNew) => {
	if (config.log_presence) {
		if (userNew.status != userOld.status) {
			if (!userNew.game)
				console.log(cDebug(" PRESENCE ") + " " + userNew.username + " is now " + userNew.status);
			else
				console.log(cDebug(" PRESENCE ") + ` ${userNew.username} is now ${userNew.status} playing ${userNew.game.name}`);
		}
	}
	...
	if (userOld.username != userNew.username) {
		bot.servers.map(ser => {
			if (ServerSettings.hasOwnProperty(ser.id) && ServerSettings[ser.id].nameChanges == true) { //if server wants name changes logged
				if (ser.members.get('id', userOld.id)) { //if user in server
					if (ServerSettings[ser.id].notifyChannel == "general")
						bot.sendMessage(ser, `[${new Date().toUTCString()}] **\`${userOld.username.replace(/@/g, '@\u200b')}\`** --> **\`${userNew.username.replace(/@/g, '@\u200b')}\`**`);
					else
						bot.sendMessage(ServerSettings[ser.id].notifyChannel, `[${new Date().toUTCString()}] **\`${userOld.username.replace(/@/g, '@\u200b')}\`** --> **\`${userNew.username.replace(/@/g, '@\u200b')}\`**`);
				}
			}
		});
	}
