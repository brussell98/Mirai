...
class Bot extends Eris.Client {
	...
	constructor(options = { }, rLogger, rChatHandler) {
		...
		this.on('error', (error, shard) => error && this.logger.error(`Mirai: Shard ${shard} error:`, error))
	...
	async updateCarbon(key) {
		try {
			const res = await axios.post('https://www.carbonitex.net/discord/data/botdata.php', {
	...
	async updateDiscordBots(key) {
		try {
			const res = await axios.post(`https://bots.discord.pw/api/bots/${this.user.id}/stats`, { server_count: this.guilds.size }, {
	...
	async updateDiscordBotsOrg(key) {
		try {
			const res = await axios.post(`https://discordbots.org/api/bots/${this.user.id}/stats`, { server_count: this.guilds.size }, {
	...
	async updateBotsOnDiscord(key) {
		try {
			const res = await axios.post(`https://bots.ondiscord.xyz/bot-api/bots/${this.user.id}/guilds`, { guildCount: this.guilds.size }, {
